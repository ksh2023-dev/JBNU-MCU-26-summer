...
2. `data/output/kci_papers.json`이 있으면 같은 객체 안의 `…En`/`…Ko` 배열 쌍에서
   한·영 키워드 쌍을 수확해 `translation_memory.json`에 더한다 (없으면 그냥 넘어간다).
...
- 조립기(9단계)가 이 파일을 읽어 교수 레코드에 **`keywordsKo` 필드로** 한글 표기를 넣는
  부분은 아직 조립기 쪽에 없다 — 병합 뒤 통합한다. 영문 원본 `keywords`는 대체하지 않는다.
  (회의 결정 2026-08-21: 화면에는 영문 키워드만 표시한다. 백엔드가 응답 `keywords`에는
  영문만 담고 `keywordsKo`는 검색 매칭에만 쓴다 — `keywords`에 한글이 섞여 있어도
  백엔드가 로드 시 걸러내므로 안전하지만, 조립기는 처음부터 `keywordsKo`로 넣는 것이 맞다.)
...
| KCI 수확 0개 | `kci_papers.json`이 없거나(첫 회차) 모양이 예상과 다름 — 오류가 아니며, KCI 수집기 모양 확정 후 `harvest_kci_pairs`를 맞춘다 |
