...
/**
 * 최근 연구 활동 교수(API ③) 화면을 그려보기 위한 mock fixture 목록입니다.
 *
 * ※ 선정 기준을 적용한 결과가 아니라, 그냥 손으로 골라 적어 둔 id 목록입니다.
 *   - 실제 API ③의 선정 기준은 '최근 논문을 낸 교수'이며, 그 판정은
 *     백엔드가 수행합니다. (데이터 계약 문서 v6 5장 3번 — 선정 기준 확정)
 *   - 아래 id 들은 그 기준으로 뽑은 것이 아닙니다. mock 데이터에는
 *     논문 정보 자체가 없으므로 최근 논문 여부를 판단할 수 없습니다.
 *     화면 배치를 확인하려고 임의로 골라 둔 테스트용 목록일 뿐입니다.
 *   - 그래서 matchScore 같은 값으로 "계산"하지도 않습니다.
 *     계산해서 뽑으면 마치 그것이 실제 선정 결과인 것처럼 보이기 때문입니다.
 *   - 백엔드가 완성되면 이 목록은 백엔드 응답으로 통째로 대체됩니다.
 */
