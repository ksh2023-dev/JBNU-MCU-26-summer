...
> 요청/응답의 **칸 이름과 구조는 v6.1과 동일**합니다. 이번 개정은 그동안 문서에 비어 있던 규칙을 계약으로 확정하는 것이 목적이며, 대부분은 이미 백엔드에 구현되어 동작하는 내용입니다.
> **단, 모든 항목이 구현 완료된 것은 아닙니다.** 특히 **논문 초록 검색**은 이번에 계약으로 확정한 사항이지만 현재 백엔드 검색 로직에는 아직 반영되지 않았습니다. 계약이 먼저 확정되고 구현이 뒤따르는 항목은 해당 위치에 별도로 표시했습니다.
